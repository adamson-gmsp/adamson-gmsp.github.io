...
valueBox(value = box3,icon = box3_icon,caption = "MSP trend from 2022", color = "lightblue")
```
...
line_T1 = cca_data %>% 
  ...
  scale_x_continuous(expand = c(0.04, 0), limits = c(BY, EY), 
                     breaks = seq(2015, 2023, 1)) + 
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(-30000, 35000), 
                     breaks = seq(-30000, 35000, 5000)) +
  scale_color_manual(values=c(MSP='seagreen3', 'peer regions'='grey')) +
  scale_size_manual(values = c(MSP=1.2, no=0.8))+
  theme(
    text = element_text(family = "Lato"),
...
bar_T1 = cca_data %>%
  ...
  coord_flip() +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
  scale_y_continuous(labels = scales::comma, limits = c(-30000,35000), breaks = seq(-30000,35000,5000))+
  xlab("") +
  ylab("") +
  theme(
    plot.background = element_rect(fill = "white"),
...
valueBox(value = box2,icon = NA,caption = "MSP Region, 2022",color = "lightblue")
```
